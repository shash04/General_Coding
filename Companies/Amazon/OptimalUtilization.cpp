...
vector<vector<int>> findOptimalPairs(vector<vector<int>>& arrA, vector<vector<int>>& arrB, int target)
{
    vector<vector<int>> result;
    
    // sort both vectors according to values
    sort(arrA.begin(), arrA.end(), mySort);
    sort(arrB.begin(), arrB.end(), mySort);
    
    int sum = 0;
    int minSum = INT_MIN;
    
    int aIdx = 0, bIdx = arrB.size() - 1;
    
    while(aIdx < arrA.size() && bIdx >= 0)
    {
        sum = arrA[aIdx][1] + arrB[bIdx][1];
        ...
        else
        {
            ...
            result.push_back({arrA[aIdx][0], arrB[bIdx][0]});      
            aIdx++;
        }
    }
...
{
    vector<vector<int>> result = findOptimalPairs(a, b, target);
