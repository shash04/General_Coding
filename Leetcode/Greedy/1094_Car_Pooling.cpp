...
class Solution {
    map<int, int> locPassMap;
    
public:
    bool carPooling(vector<vector<int>>& trips, int capacity) {
        for(int i = 0; i < trips.size(); i++)
        {
            int numPas = trips[i][0];
            int startLoc = trips[i][1], endLoc = trips[i][2];
            
            locPassMap[startLoc] += numPas;
            locPassMap[endLoc]   -= numPas;
        }
        
        int currPassengers = 0;
        
        for(auto& entry : locPassMap)
        {
            currPassengers += entry.second;
