...
public:
    ...
    bool book(int start, int end) {
        timeline[start]++;              // Increment the timeline start
        timeline[end]--;                // Decrement the timeline end
        int booked = 0;
        
        for (auto &iter : timeline) {
            booked += iter.second;
            if (booked == 3)            // As starts are sorted in timeline, 3 consecutive starts would make booked == 3
            {
                timeline[start]--;
                timeline[end]++;
                return false;
