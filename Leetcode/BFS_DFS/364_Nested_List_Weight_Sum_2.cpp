...
class Solution {
    ...
    void fillMap(NestedInteger& currNI, int currDepth, int& maxDepth)
    {
        maxDepth = max(maxDepth, currDepth);
        
        if(currNI.isInteger())
        {
            depthMap[currDepth] += currNI.getInteger();
        }
        else
        {
            for(auto& ni : currNI.getList())
            {
                fillMap(ni, currDepth + 1, maxDepth);
            }
        }
...
public:
    int depthSumInverse(vector<NestedInteger>& nestedList) {
        ...
        {
            fillMap(ni, 0, maxDepth);
        }
        
        for(auto& entry : depthMap)
        {
            retAns += entry.second * (maxDepth + 1 - entry.first);
        }
