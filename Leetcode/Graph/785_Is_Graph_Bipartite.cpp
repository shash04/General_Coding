...
public:
    bool isBipartite(vector<vector<int>>& graph) {       
        ...
        // color = 0 (unvisited) : -1 (group A) : 1 (group B)
        ...
        for(int i = 0; i < graph.size(); i++)
        {
            ...
            // doesn't matter if we assign 1 or -1
            color[i] = 1;
            ...
            while(!BfsQ.empty())
            {
                ...
                while(qSize--)
                {
                    ...
                    {
                        // If the neighbor has not yet visited, assign opp grp and add to queue
                        if(color[neighbor] == 0)
                        {
                            color[neighbor] = -color[curNode];
                            BfsQ.push(neighbor);
                        }
                        // if the joining node falls in same grp, return false
